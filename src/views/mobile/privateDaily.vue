...
export default {
  ...
  methods: {
    ...
    approvalDaily(index) {
      ...
      if (valid) {
        const param = {
          idPrivateDaily: this.form.idPrivateDaily,
          idResultType: this.form.idResultType,
          remark: this.form.remark,
          idUser: this.idUser
        }
        APPROVAL.approvalPrivateDaily(param)
        .then(res => {
          if (res.data.status === 200) {
            Toast.success('审批成功')
    ...
    // 获取私账收支明细
    getPrivateDailyByDate(val) {
      API.getPrivateDailyByDate({
        date: val.createTime
      }).then(res => {
        if (res.data.status === 200) {
          this.flowRecordDta = res.data.datas
          this.form.idPrivateDaily = val.idPrivateDaily
        }
      })
    },
    // 获取请款记录列表
    getTableData() {
      ...
      API.getPrivateDailyByPage(params)
      .then(res => {
        if (res.data.status === 200) {
    ...
    getApprovalCount() {
      API.selectDailyByState({
        ...
        if (res.data.status === 200) {
          let tmpData = res.data.datas
          this.unApprovalCount = tmpData.countPri
          this.approvalTotal = tmpData.approvalTotalPri
        }
      })
    }
  }
