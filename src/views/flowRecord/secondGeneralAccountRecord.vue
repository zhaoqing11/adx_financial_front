                  ...
                  <div class="card card-block card-stretch">
                    <div class="card-body p-0">
                      ...
                      <div class="pagination">
                        <Pagination 
                        v-show="privateTotalPage > 0" 
                        :total="privateTotalPage" 
                        :page.sync="pageNum" 
                        :limit.sync="pageSize" 
...
export default {
  ...
  methods: {
    ...
    // 分页获取流水记录
    changeToPage() {
      this.privateTotalPage = this.tableData.length
      this.privateRemainSumList = JSON.parse(JSON.stringify(this.tableData))
      this.privateRemainSumList = this.privateRemainSumList.slice((this.pageNum - 1) * this.pageSize, this.pageNum * this.pageSize)
    },
    // 获取个人请款记录列表
    getTableData() {
      let startTime = ''
      ...
      .then(res => {
        if (res.data.status === 200) {
          ...
          this.tableData = tmpData.generalAccountMpTwo.datas
          this.changeToPage()
        }
      })
    }
