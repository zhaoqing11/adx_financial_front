  ...
  methods: {
    ...
    approvalDaily(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          const param = {
            idPubGeneralDaily: this.form.idPubGeneralDaily,
            idResultType: this.form.idResultType,
            remark: this.form.remark,
            idUser: getUserId()
          }
          APPROVAL.approvalPubGeneral(param)
          .then(res => {
            if (res.data.status === 200) {
    ...
    showEditDaily(data) {
      this.idDaily = data.idPubGeneralDaily
    ...
    getPubGeneralDailyByDate() {
      let data = this.publicDailyForm
      API.getPubGeneralcDailyByDate({
        date: formatDate2(data.createTime)
      }).then(res => {
        if (res.data.status === 200) {
          this.flowRecordDta = res.data.datas
          this.form.idPubGeneralDaily = data.idPubGeneralDaily
        }
      })
    },
    ...
    getTableData() {
      ...
      API.getPubGeneralDailyByPage(params)
      .then(res => {
        if (res.data.status === 200) {
