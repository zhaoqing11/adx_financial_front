  ...
  methods: {
    ...
    approvalDaily(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          const param = {
            idSecondGeneralAccountDaily: this.form.idSecondGeneralAccountDaily,
            idResultType: this.form.idResultType,
            remark: this.form.remark,
            idUser: getUserId()
          }
          APPROVAL.approvalSecondGeneralDaily(param)
          .then(res => {
            if (res.data.status === 200) {
    ...
    showEditDaily(data) {
      this.idDaily = data.idSecondGeneralAccountDaily
    ...
    getGeneralDailyByDate() {
      ...
      API.getSecondGeneralDailyByDate({
        date: formatDate2(data.createTime)
      }).then(res => {
        if (res.data.status === 200) {
          this.flowRecordDta = res.data.datas
          this.form.idSecondGeneralAccountDaily = data.idSecondGeneralAccountDaily
        }
      })
    },
    ...
    getTableData() {
      ...
      API.getSecondGeneralAccountDaily(params)
      .then(res => {
        if (res.data.status === 200) {
